...
| G29 | `IgnixaImportResourceParser.CheckConditionalReferenceInResource` detects conditional references for `$import` via `IReferenceMetadataProvider`-declared element paths + FHIRPath (`{elementPath}.reference.contains('?')`), walked only from the metadata provider's per-resource-type field list. This misses conditional references inside extensions (`extension[].valueReference`) and contained resources, which aren't enumerated by that metadata walk. Detection-only (no data-corruption risk — a missed conditional reference is imported as a literal, unresolved string, not silently misresolved); found as a side-effect of US-16 Task 1 research, not itself part of that task. | **Low** | 2 (detection-only gap in `$import`) | Fix opportunistically: `IgnixaReferenceScanner` (US-16 Task 1, schema-typed tree walk, not metadata-path-driven) already visits extensions and contained resources and would close this gap outright if swapped in for the FHIRPath-metadata approach | US-16 Task 1 design research (`task-1-report.md`) |
