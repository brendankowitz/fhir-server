...
- The standalone project `src/Microsoft.Health.Fhir.Ignixa/` (7 source files: serializer, resource element, schema context interface, FHIRPath provider) duplicates files under `src/Microsoft.Health.Fhir.Shared.Core/Ignixa/` with **identical namespaces** (`Microsoft.Health.Fhir.Ignixa`, `Microsoft.Health.Fhir.Core.Features.Persistence`). **[Corrected by the 2026-07-08 adversarial review — the original bullet had the live/dead attribution inverted.]** The standalone project is the **live** one: `Microsoft.Health.Fhir.Shared.Core.projitems:11` references it, and the projitems compiles only the two formatters + `ServiceCollectionExtensions` from the `Shared.Core/Ignixa` folder (`projitems:14-16`). The other 7 disk files in that folder are **uncompiled duplicates** (6 byte-identical, `FhirPath/IgnixaCompiledFhirPath.cs` already drifted) — they are the dead weight to delete. The type-identity trap (same full type name compiled into two assemblies) is latent only; it would materialize if the dead copies were ever added to compilation. The shim-minimization audit §3.5 and ADR-2607 G26/US-23 carry the correct account.
